-- Store Procedure cho phiếu nhập, phiếu xuất, ctpn, ctpx, pbsp, kho_pbsp
...
BEGIN
	...
	IF @maKho = 'HN'
	BEGIN
		-- Kiểm tra xem nhân viên có tồn tại trên linked server LINK1 không
		...
		BEGIN
			...
			LEFT JOIN LINK1.phonestore.DBO.KHO_PBSP AS T2 ON T1.maPBSP = T2.maPBSP
			WHERE T2.maPBSP IS NULL;

			-- Thực thi procedure ThemPhieuNhap trên LINK1 với biến đã nối chuỗi
			IF @InvalidProductCount > 0
			BEGIN
				-- Báo lỗi nếu có mã sản phẩm không tồn tại trên LINK1
				RAISERROR('Có mã sản phẩm không hợp lệ trong chi tiết phiếu nhập cho kho HN.', 16, 1);
				RETURN 1;
			END
			Exec LINK1.phonestore.DBO.ThemPhieuNhap @remoteMaPN, @NgayTao, @tongTien, @trangThai, @maNV, @maKho, @maNCC, @ChiTietJSON;
		END
		ELSE
		BEGIN
			-- Báo lỗi nếu mã nhân viên không hợp lệ trên LINK1
	...
	ELSE IF @maKho = 'DN'
	BEGIN
		-- Kiểm tra xem nhân viên có tồn tại trên linked server LINK2 không
		IF EXISTS (SELECT 1 FROM LINK2.phonestore.DBO.NHANVIEN WHERE maNV = @maNV)
		BEGIN
			-- Nối chuỗi mã phiếu nhập
			SET @remoteMaPN = 'PN_DN' + @maPN;
			-- Thực thi procedure ThemPhieuNhap trên LINK2 với biến đã nối chuỗi
			Exec LINK2.phonestore.DBO.ThemPhieuNhap @remoteMaPN, @NgayTao, @tongTien, @trangThai, @maNV, @maKho, @maNCC, @ChiTietJSON;
		END
		ELSE
		BEGIN
			-- Báo lỗi nếu mã nhân viên không hợp lệ trên LINK2
...
INSERT INTO @ChiTietNhap (soLuong, giaNhap, maPBSP) VALUES
(10, 50000, 'PBSP001'),
(5, 100000, 'PBSP004'),
(10, 50000, 'PBSP005');

Exec themPN_Goc '003', '2025-02-02',100000, N'Đã nhận hàng', 'NV005', 'HN', 'NCC002', @ChiTietNhap;

SELECT * FROM LINK1.phonestore.DBO.PHIEUNHAP
SELECT * FROM LINK1.phonestore.DBO.CTPN
SELECT * FROM LINK1.phonestore.DBO.KHO_PBSP

DELETE FROM LINK1.phonestore.DBO.PHIEUNHAP WHERE maPN = 'PN_HN003'


-- 2. Cập nhật phiếu nhập

-- =============================================
-- Script này cần chạy trên các LINKED SERVER (LINK1, LINK2, LINK3)
...
BEGIN
	...
	IF @maKho = 'HN'
		SET @linkedServerName = 'LINK1';
	ELSE IF @maKho = 'DN'
		SET @linkedServerName = 'LINK2';
	ELSE IF @maKho = 'HCM'
...
-- Kết quả mong đợi: Cập nhật thành công chi tiết và tổng tiền trên LINK1. Trạng thái vẫn là 'Chờ xác nhận'.
...
-- Kết quả mong đợi: Cập nhật thành công chi tiết (nếu có) và đổi trạng thái trên LINK1 thành 'Đã xác nhận'. Tồn kho KHÔNG thay đổi.
*/
...
EXEC CapNhatPN_Goc
	...
	@maCV = 'CV001', -- Quản lý Kho
	@ChiTietPhieuNhap = @ChiTietNhapCapNhat; -- Có thể gửi chi tiết để cập nhật cùng lúc hoặc gửi rỗng (thực tế nên gửi đầy đủ chi tiết hiện tại)
-- Kết quả mong đợi: Cập nhật thành công chi tiết và tổng tiền trên LINK1. Trạng thái vẫn là 'Đã xác nhận'.
...
EXEC CapNhatPN_Goc
	...
	@maCV = 'QL', -- Quản lý Kho
	@ChiTietPhieuNhap = @ChiTietNhapCapNhat; -- Có thể gửi chi tiết để cập nhật cùng lúc hoặc gửi rỗng (thực tế nên gửi đầy đủ chi tiết hiện tại)
-- Kết quả mong đợi: Cập nhật thành công chi tiết (nếu có), đổi trạng thái trên LINK1 thành 'Đã nhận hàng'. Tồn kho SẼ được cập nhật (tăng) theo chi tiết đã gửi.
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				RETURN 1;
			END
			Exec LINK1.phonestore.DBO.ThemPhieuXuat @remoteMaPX, @NgayTao, @diaChi, @tongTien, @httt, @trangThai, @maNV, @maKho, @maKH, @ChiTietJSON;
		END
		ELSE
			BEGIN
				-- Báo lỗi nếu mã nhân viên không hợp lệ trên LINK1
	...
	BEGIN
		...
		BEGIN
			...
			-- Thực thi procedure ThemPhieuNhap trên LINK2 với biến đã nối chuỗi
			Exec LINK2.phonestore.DBO.ThemPhieuXuat @remoteMaPX, @NgayTao, @diaChi, @tongTien, @httt, @trangThai, @maNV, @maKho, @maKH, @ChiTietJSON
		END
		ELSE
		BEGIN
			-- Báo lỗi nếu mã nhân viên không hợp lệ trên LINK2
...
SELECT * FROM LINK1.phonestore.DBO.PHIEUXUAT

SELECT * FROM LINK1.phonestore.DBO.CTPX

SELECT * FROM LINK1.phonestore.DBO.KHO_PBSP

DELETE FROM LINK1.phonestore.DBO.PHIEUXUAT WHERE maPX = 'PX_HN003'
...
-- Các lệnh SELECT kiểm tra (chạy trên Server Gốc, thay LINK1 nếu cần)
SELECT * FROM LINK1.phonestore.DBO.PHIEUXUAT;
SELECT * FROM LINK1.phonestore.DBO.CTPX;
SELECT * FROM LINK1.phonestore.DBO.KHO_PBSP;
...
EXEC XoaPN_Goc
	...
	@maKho = 'HN',
	@maCV = 'CV003'; -- Nhân viên
-- Kết quả mong đợi: Xóa thành công PN_HN006 và chi tiết của nó trên LINK1.
...
EXEC XoaPX_Goc
	...
	@maKho = 'HN',
	@maCV = 'CV003'; -- Nhân viên
-- Kết quả mong đợi: Xóa thành công PX_HN004 và chi tiết của nó trên LINK1.
