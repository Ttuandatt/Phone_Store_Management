...
package Components.dashboardApp.raven.chart.line;
...
public class LineChart extends PlotChart {
    ...
    private void createColumn(Graphics2D g2, int row, int width, int height) {
        ...
        if (mapSpline.containsKey(row)) {
            mapSpline.get(row).render(g2, raven.chart.line.LineChart.ChartType.LINE, point);
        } else {
            spline = new Spline();
            spline.render(g2, raven.chart.line.LineChart.ChartType.LINE, point);
    ...
    private void createSelectedIndex(Graphics2D g2, int height) {
        if (selectedIndex >= 0) {
            ...
            if (mapSpline.containsKey(0)) {
                ...
                SplinePoint point = mapSpline.get(0).getSpline(raven.chart.line.LineChart.ChartType.LINE, index);
            ...
            for (int i = 0; i < rows; i++) {
                if (mapSpline.containsKey(i)) {
                    SplinePoint point = mapSpline.get(i).getSpline(raven.chart.line.LineChart.ChartType.LINE, index);
                    g2.setColor(chartColor.getColor(i));
