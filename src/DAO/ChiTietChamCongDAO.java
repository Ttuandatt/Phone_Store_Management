...
public class ChiTietChamCongDAO implements DAOInterface<ChiTietChamCongDTO> {
	...
	public ArrayList<ChiTietChamCongDTO> getThongTinTangCa(String maBCC) {
		...
		try {
			...
			String query = "select * from chitietchamcong where mabcc=? and soGioOT>0";
			PreparedStatement ps = jdbc.getConnection().prepareStatement(query);
			...
			while (rs.next()) {
				...
				ctcc.setMaCTCC(rs.getString("maCTCC"));
	...
	public ArrayList<ChiTietChamCongDTO> getThongTinNgayNghi(String maBCC) {
		...
		try {
			...
			String query = "SELECT * " + "FROM ChiTietChamCong " + "WHERE maBCC = ? AND loaiChamCong LIKE N'%Nghỉ%'";
			PreparedStatement ps = jdbc.getConnection().prepareStatement(query);
			ps.setString(1, maBCC);
	...
	public int deleteByMaCC(String macc) {
		...
		try {
			...
			if (result > 0)
				...
				System.out.println("Không tồn tại chi tiết chấm công theo maBCC");
