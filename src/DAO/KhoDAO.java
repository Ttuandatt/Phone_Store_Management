...
public class KhoDAO implements DAOInterface<KhoDTO>{
  ...
  public String getMaNVQuanLyKho(String maKho) {
    String maNV = null;
    String query = "SELECT nv.maNV FROM nhanvien nv " +
                   "WHERE nv.maCV = 'CV001' AND nv.chiNhanh = ?";

    try {
        if (jdbc.isConnectionClosed()) {
            jdbc.openConnection();
        }
    ...
    try {
        String sql = "SELECT pbsp.maPBSP, pbsp.mauSac, pbsp.RAM, pbsp.ROM, pbsp.giaBan, pbsp.soLuong, pbsp.trangThai, pbsp.maSP " +
                     "FROM pbsp JOIN KHO_PBSP kps ON pbsp.maPBSP = kps.maPBSP " +
                     "WHERE kps.maKho = ?";
