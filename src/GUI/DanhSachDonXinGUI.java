...
//             PopupNhaCungCap frameThem = new PopupNhaCungCap("Thêm nhà cung cấp", "THÊM NHÀ CUNG CẤP","add");
	...
	private void deletePerformed(JTable tb) {
		...
		if (selectedRow != -1) {
			int dialogResult = JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa nhà cung cấp?", "Xác nhận xóa",
					JOptionPane.OK_CANCEL_OPTION);
			if (dialogResult == JOptionPane.OK_OPTION) {
				String mancc = (String) donXinModel.getValueAt(selectedRow, 0);

				// Tạo đối tượng DTO để truyền vào BUS
				NhaCungCapDTO ncc = new NhaCungCapDTO();
				ncc.setMaNCC(mancc);

//				String message = donXinBUS.deleteNhaCungCap(ncc);
//				if ("Xóa nhà cung cấp thành công".equals(message)) {
//					donXinModel.removeRow(selectedRow);
//					donXinArr.remove(selectedRow);
//				}
//				JOptionPane.showMessageDialog(null, message);
			}

		} else {
			JOptionPane.showMessageDialog(null, "Vui lòng chọn 1 dòng để xóa", "Thông báo",
					JOptionPane.WARNING_MESSAGE);
		}
	}

	private void updatePerformed(JTable tb) {
		int selected = donXinTable.getSelectedRow();
		if (selected != -1) {
//            PopupNhaCungCap frameSua = new PopupNhaCungCap("Sửa nhà cung cấp", "SỬA NHÀ CUNG CẤP", "fix");
		} else
			JOptionPane.showMessageDialog(null, "Vui lòng chọn một dòng để sửa", "Thông báo",
					JOptionPane.WARNING_MESSAGE);
	}
	...
	private void searchPerformed(JTable tb) {
		...
		if (!searchContent.isEmpty()) {
			...
			boolean found = false;
//            for(DonXinNghiDTO ncc: donXinArr){
...
//            }
			...
			for (NhaCungCapDTO nhaCungCap : dsTimKiem) {
				Object[] row = { nhaCungCap.getMaNCC(), nhaCungCap.getTenNCC(), nhaCungCap.getDiaChi(),
						nhaCungCap.getSdt(), nhaCungCap.getEmail(), nhaCungCap.getTrangthai() };
	...
	private void xemPerformed(JTable tb) {
		...
		if (selectedRow != -1) {
			...
			pnTrangThai.add(tftrangthai);

			String maNCC = donXinTable.getValueAt(selectedRow, 0).toString();
