		...
		btnUpdate.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (selectedRow != -1) {
					...
					KhachHangDTO selectedKH = new KhachHangDTO(maKH, hoTen, ngaySinh, gioiTinh, diaChi, sdt, email,
							trangThai);
					// Mở dialog cập nhật
					updateKhachHangDialog(selectedKH);
				} else {
					JOptionPane.showMessageDialog(null, "Vui lòng chọn một khách hàng để cập nhật!", "Thông báo",
...
//        btnExcel.setFocusPainted(false);
		...
		btnPrint.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JTable table = pr.getTableKhachHangFromDatabase();
				if (table != null) {
					pr.printTableKhachHang(table); // Thực hiện in
				} else {
					JOptionPane.showMessageDialog(null, "Không thể lấy dữ liệu từ bảng khách hàng.", "Lỗi",
...
//            	refreshList();
	...
	private void newKhachHangDialog() {
		...
		gbc.gridx = 0;
		gbc.gridy = y;
		dialog.add(lblMaKH, gbc);
		gbc.gridx = 1;
		dialog.add(txtMaKH, gbc);

		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblHoTen, gbc);
		gbc.gridx = 1;
		dialog.add(txtHoTen, gbc);

		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblNgaySinh, gbc);
		gbc.gridx = 1;
		dialog.add(dateChooser, gbc);

		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblGioiTinh, gbc);
		gbc.gridx = 1;
		dialog.add(cbGioiTinh, gbc);

		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblDiaChi, gbc);
		gbc.gridx = 1;
		dialog.add(txtDiaChi, gbc);

		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblSDT, gbc);
		gbc.gridx = 1;
		...
		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblTrangThai, gbc);
		gbc.gridx = 1;
		dialog.add(cbTrangThai, gbc);
		...
		btnSave.addActionListener(e -> {
			try {
				...
				String ngaySinh = ud != null ? new java.text.SimpleDateFormat("yyyy-MM-dd").format(ud) : "";
				...
				KhachHangDTO kh = new KhachHangDTO(maKH, hoTen, ngaySinh, gioiTinh, diaChi, sdt, email, trangThai);
	...
	private void updateKhachHangDialog(KhachHangDTO selectedKH) {
		...
		dateChooser.setDateFormatString("yyyy-MM-dd");
		try {
			java.util.Date d = new java.text.SimpleDateFormat("yyyy-MM-dd").parse(selectedKH.getNgaySinh());
		...
		btnUpdate.addActionListener(e -> {
			try {
				...
				ex.printStackTrace();
				JOptionPane.showMessageDialog(dialog, "Lỗi khi cập nhật khách hàng!", "Lỗi", JOptionPane.ERROR_MESSAGE);
			}
		});
	...
	public void deleteKhachHangDialog(JTable customersTable) {
		...
		if (selectedRow != -1) {
			int dialogResult = JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa khách hàng này?",
	...
	private void detailKhachHangDialog(String maKH) {
		...
		dialog.add(lblNgaySinh, gbc);
		gbc.gridx = 1;
		dialog.add(txtNgaySinh, gbc);

		gbc.gridx = 0;
		gbc.gridy = ++y;
		dialog.add(lblGioiTinh, gbc);
		gbc.gridx = 1;
		dialog.add(txtGioiTinh, gbc);
	...
	private void searchPerformed(JTable tb) {
		...
		if (!searchContent.isEmpty()) {
			...
			for (KhachHangDTO kh : dsTimKiem) {
				Object[] row = { kh.getMaKH(), kh.getHoTen(), kh.getNgaySinh(), kh.getGioiTinh(), kh.getDiaChi(),
						kh.getSdt(), kh.getEmail(), kh.getTrangThai() };
				model.addRow(row);
			}
		} else {
